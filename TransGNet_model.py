...
def drop_path(x, drop_prob: float = 0., training: bool = False):  # stochastic depth decay rule
...
def Xrd_Embedding(x, xrd_length, embed_dim):

    batch_size = x.shape[0]
    x = torch.reshape(x, (batch_size, xrd_length //
                      embed_dim, embed_dim))  # 分割嵌入
    return x
...
class VisionTransformer(nn.Module):
    def __init__(self, xrd_length=2000, num_classes=1,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(
            1, (xrd_length//embed_dim) + 1, embed_dim))
    ...
    def forward(self, data):
        # 考虑同时放入data
        # transformer数据的forward过程
        # [B , xrd_length] --> [B , xrd_length/embed_dim , embed_dim]
        x = data.ms_spec
        x = Xrd_Embedding(x, self.xrd_length, self.embed_dim)
...
def xrd_vit_model_CryClassify(num_classes: int = 1):

    model = VisionTransformer(xrd_length=2000,
                              embed_dim=40,
                              depth=12,
                              num_heads=2,
